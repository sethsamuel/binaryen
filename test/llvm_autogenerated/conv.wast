(module
  (memory 1
    (segment 4 "\10\04\00\00")
  )
  (export "memory" memory)
  (type $0 (func (param i64) (result i32)))
  (type $1 (func (param i32) (result i64)))
  (type $2 (func (param f32) (result i32)))
  (type $3 (func (param f64) (result i32)))
  (type $4 (func (param f32) (result i64)))
  (type $5 (func (param f64) (result i64)))
  (type $6 (func (param i32) (result f32)))
  (type $7 (func (param i32) (result f64)))
  (type $8 (func (param i64) (result f32)))
  (type $9 (func (param i64) (result f64)))
  (type $10 (func (param f32) (result f64)))
  (type $11 (func (param f64) (result f32)))
  (export "i32_wrap_i64" $i32_wrap_i64)
  (export "i64_extend_s_i32" $i64_extend_s_i32)
  (export "i64_extend_u_i32" $i64_extend_u_i32)
  (export "i32_trunc_s_f32" $i32_trunc_s_f32)
  (export "i32_trunc_u_f32" $i32_trunc_u_f32)
  (export "i32_trunc_s_f64" $i32_trunc_s_f64)
  (export "i32_trunc_u_f64" $i32_trunc_u_f64)
  (export "i64_trunc_s_f32" $i64_trunc_s_f32)
  (export "i64_trunc_u_f32" $i64_trunc_u_f32)
  (export "i64_trunc_s_f64" $i64_trunc_s_f64)
  (export "i64_trunc_u_f64" $i64_trunc_u_f64)
  (export "f32_convert_s_i32" $f32_convert_s_i32)
  (export "f32_convert_u_i32" $f32_convert_u_i32)
  (export "f64_convert_s_i32" $f64_convert_s_i32)
  (export "f64_convert_u_i32" $f64_convert_u_i32)
  (export "f32_convert_s_i64" $f32_convert_s_i64)
  (export "f32_convert_u_i64" $f32_convert_u_i64)
  (export "f64_convert_s_i64" $f64_convert_s_i64)
  (export "f64_convert_u_i64" $f64_convert_u_i64)
  (export "f64_promote_f32" $f64_promote_f32)
  (export "f32_demote_f64" $f32_demote_f64)
  (export "anyext" $anyext)
  (export "bitcast_i32_to_float" $bitcast_i32_to_float)
  (export "bitcast_float_to_i32" $bitcast_float_to_i32)
  (export "bitcast_i64_to_double" $bitcast_i64_to_double)
  (export "bitcast_double_to_i64" $bitcast_double_to_i64)
  (func $i32_wrap_i64 (type $0) (param $0 i64) (result i32)
    (return
      (i32.wrap/i64
        (get_local $0)
      )
    )
  )
  (func $i64_extend_s_i32 (type $1) (param $0 i32) (result i64)
    (return
      (i64.extend_s/i32
        (get_local $0)
      )
    )
  )
  (func $i64_extend_u_i32 (type $1) (param $0 i32) (result i64)
    (return
      (i64.extend_u/i32
        (get_local $0)
      )
    )
  )
  (func $i32_trunc_s_f32 (type $2) (param $0 f32) (result i32)
    (return
      (i32.trunc_s/f32
        (get_local $0)
      )
    )
  )
  (func $i32_trunc_u_f32 (type $2) (param $0 f32) (result i32)
    (return
      (i32.trunc_u/f32
        (get_local $0)
      )
    )
  )
  (func $i32_trunc_s_f64 (type $3) (param $0 f64) (result i32)
    (return
      (i32.trunc_s/f64
        (get_local $0)
      )
    )
  )
  (func $i32_trunc_u_f64 (type $3) (param $0 f64) (result i32)
    (return
      (i32.trunc_u/f64
        (get_local $0)
      )
    )
  )
  (func $i64_trunc_s_f32 (type $4) (param $0 f32) (result i64)
    (return
      (i64.trunc_s/f32
        (get_local $0)
      )
    )
  )
  (func $i64_trunc_u_f32 (type $4) (param $0 f32) (result i64)
    (return
      (i64.trunc_u/f32
        (get_local $0)
      )
    )
  )
  (func $i64_trunc_s_f64 (type $5) (param $0 f64) (result i64)
    (return
      (i64.trunc_s/f64
        (get_local $0)
      )
    )
  )
  (func $i64_trunc_u_f64 (type $5) (param $0 f64) (result i64)
    (return
      (i64.trunc_u/f64
        (get_local $0)
      )
    )
  )
  (func $f32_convert_s_i32 (type $6) (param $0 i32) (result f32)
    (return
      (f32.convert_s/i32
        (get_local $0)
      )
    )
  )
  (func $f32_convert_u_i32 (type $6) (param $0 i32) (result f32)
    (return
      (f32.convert_u/i32
        (get_local $0)
      )
    )
  )
  (func $f64_convert_s_i32 (type $7) (param $0 i32) (result f64)
    (return
      (f64.convert_s/i32
        (get_local $0)
      )
    )
  )
  (func $f64_convert_u_i32 (type $7) (param $0 i32) (result f64)
    (return
      (f64.convert_u/i32
        (get_local $0)
      )
    )
  )
  (func $f32_convert_s_i64 (type $8) (param $0 i64) (result f32)
    (return
      (f32.convert_s/i64
        (get_local $0)
      )
    )
  )
  (func $f32_convert_u_i64 (type $8) (param $0 i64) (result f32)
    (return
      (f32.convert_u/i64
        (get_local $0)
      )
    )
  )
  (func $f64_convert_s_i64 (type $9) (param $0 i64) (result f64)
    (return
      (f64.convert_s/i64
        (get_local $0)
      )
    )
  )
  (func $f64_convert_u_i64 (type $9) (param $0 i64) (result f64)
    (return
      (f64.convert_u/i64
        (get_local $0)
      )
    )
  )
  (func $f64_promote_f32 (type $10) (param $0 f32) (result f64)
    (return
      (f64.promote/f32
        (get_local $0)
      )
    )
  )
  (func $f32_demote_f64 (type $11) (param $0 f64) (result f32)
    (return
      (f32.demote/f64
        (get_local $0)
      )
    )
  )
  (func $anyext (type $1) (param $0 i32) (result i64)
    (return
      (i64.shl
        (i64.extend_u/i32
          (get_local $0)
        )
        (i64.const 32)
      )
    )
  )
  (func $bitcast_i32_to_float (type $6) (param $0 i32) (result f32)
    (return
      (f32.reinterpret/i32
        (get_local $0)
      )
    )
  )
  (func $bitcast_float_to_i32 (type $2) (param $0 f32) (result i32)
    (return
      (i32.reinterpret/f32
        (get_local $0)
      )
    )
  )
  (func $bitcast_i64_to_double (type $9) (param $0 i64) (result f64)
    (return
      (f64.reinterpret/i64
        (get_local $0)
      )
    )
  )
  (func $bitcast_double_to_i64 (type $5) (param $0 f64) (result i64)
    (return
      (i64.reinterpret/f64
        (get_local $0)
      )
    )
  )
)
;; METADATA: { "asmConsts": {},"staticBump": 1040, "initializers": [] }
